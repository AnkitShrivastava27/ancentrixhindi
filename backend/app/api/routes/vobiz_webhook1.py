    ...
    if transcript:
        ...
        if company:
            try:
                ...
                analysis = await llm_service.analyze_call(
                    transcript,
                    f"{company.name} | industry={company.industry or ''} | description={company.description or ''} | active_product={company.active_product or ''}",
                    lead_ctx,
                )
            except Exception as e:
                logger.error(f"Analysis error: {e}")
    ...
    async with AsyncSessionLocal() as db:
        ...
        if lead_id:
            ...
            if lead:
                ...
                if ki:
                    lead.key_info = {**(lead.key_info or {}), **{k: v for k, v in ki.items() if v}}
                # Keep lead notes short and actionable: one sentence only.
                ki = analysis.get("key_info") or {}
                intent = analysis.get("intent") or ""
                pieces = []
                if ki.get("product"): pieces.append(str(ki["product"]))
                if ki.get("location"): pieces.append(f"in {ki['location']}")
                if ki.get("budget"): pieces.append(f"budget {ki['budget']}")
                if ki.get("timeline"): pieces.append(f"timeline {ki['timeline']}")
                if intent in ("site_visit", "meeting", "demo"):
                    note = "Lead wants a site visit/meeting; admin should confirm the slot manually."
                elif intent == "wants_callback" or analysis.get("follow_up_required"):
                    note = "Lead requested a follow-up; human team should contact them."
                elif pieces:
                    note = "Lead interested in " + " ".join(pieces) + "."
                elif analysis.get("summary"):
                    note = str(analysis.get("summary")).strip()
                else:
                    note = "Call completed; follow up if required."
                note = " ".join(note.split())[:240].rstrip(" ,;-")
                existing_notes = (lead.notes or "").strip()
                stamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
                new_note = f"[{stamp}] {note}"

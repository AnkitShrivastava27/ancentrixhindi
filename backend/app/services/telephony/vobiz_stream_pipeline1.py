...
async def run_vobiz_stream_pipeline(
    ...
    product_focus = product_focus or getattr(company, "active_product", None)
    ...
    _END_PHRASES = (
        ...
        "goodbye", "good bye", "bye", "bas itna hi", "bas ho gaya",
        "theek hai bye", "thik hai bye", "nahi chahiye, bye", "stop calling",
        "call mat karna", "aur kuch nahi", "that's all", "that is all",
    )
    ...
    async def _finish_call_after_farewell():
        farewell = "Theek hai ji, thank you. Aapka din achha rahe."
        try:
            await task.queue_frames([TTSSpeakFrame(text=farewell)])
            await asyncio.sleep(2.0)
            from app.services.telephony.vobiz_service import vobiz_service
            await vobiz_service.hangup(call_uuid, company)
            await task.queue_frames([EndFrame()])
        except Exception as exc:
            logger.warning(f"Explicit end-call cleanup failed | call_uuid={call_uuid[:12]} | {exc}")
        ...
        async def process_frame(self, frame, direction: FrameDirection):
            ...
            if self._kind == "ai":
                ...
                if text and isinstance(frame, (TranscriptionFrame, TextFrame)):
                    ...
                    if _is_explicit_end(text):
                        logger.info(f"Explicit caller end request | call_uuid={call_uuid[:12]} | text={text[:100]}")
